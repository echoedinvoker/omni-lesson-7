...
def setup(db: og.Database):
    ...
    state.box_path_list = []
    state.asset_path_list = ['cardbox.usd', 'defect_cardbox.usd']
    state.count = 1
    state.last_time = time.time()
    state.manager = omni.kit.app.get_app().get_extension_manager()
    state.extension_data_path = os.path.join(state.manager.get_extension_path_by_module("refined_defect_detection"), "data")
...
def compute(db: og.Database):
    ...
    if time.time() - state.last_time > 8:
        prim_path = f'/World/cardbox_{index}_{str(state.count).zfill(2)}'
        state.box_path_list.append(prim_path)

        asset_path = os.path.join(state.extension_data_path, random.choice(state.asset_path_list))
